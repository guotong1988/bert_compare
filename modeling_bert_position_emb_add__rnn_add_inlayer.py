...
def my_lstm_layer(input_reps, lstm_dim=int(768 / 2), input_lengths=None, scope_name="my_rnn", reuse=False,
                  is_training=True,
...
class BertModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            self.embedding_output2 = tf.cast(tf.expand_dims(input_mask, -1), tf.float32) * self.embedding_output
            self.embedding_output2 = my_lstm_layer(self.embedding_output2, is_training=is_training)[2]
            self.embedding_output2 = tf.cast(tf.expand_dims(input_mask, -1), tf.float32) * self.embedding_output2
            self.embedding_output3 = self.embedding_output2 + self.embedding_output
            ...
            with tf.variable_scope("encoder"):
                ...
                self.all_encoder_layers = transformer_model(
                    input_tensor=self.embedding_output3,
...
def transformer_model(input_tensor,
    ...
    for layer_idx in range(num_hidden_layers):
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            if layer_idx >= 1:  # FIXME-NEW-20210811
                layer_input2 = tf.cast(tf.expand_dims(sequence_mask, -1), tf.float32) * tf.reshape(layer_input,
                                                                                                   [batch_size, seq_length,
                                                                                                    hidden_size])
                layer_input2 = my_lstm_layer(layer_input2, is_training=is_training)[2]
                layer_input2 = tf.cast(tf.expand_dims(sequence_mask, -1), tf.float32) * layer_input2

                layer_input3 = layer_input2 + tf.reshape(layer_input, [batch_size, seq_length, hidden_size])
            else:  # FIXME-NEW-20210811
                layer_input3 = layer_input

            layer_input3 = reshape_to_matrix(layer_input3)
            ...
            if layer_idx >= 1:
                ...
                embedding_output = reshape_to_matrix(embedding_output)
                layer_input = layer_input + embedding_output
                layer_input4 = layer_input + layer_input3
            else:
                layer_input4 = layer_input

            with tf.variable_scope("attention"):
                attention_heads = []
                with tf.variable_scope("self"):
                    attention_head = attention_layer(
                        from_tensor=layer_input4,
                        to_tensor=layer_input4,
                        attention_mask=attention_mask,
