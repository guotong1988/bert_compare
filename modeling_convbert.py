...
# global_step/sec: 2.86
# loss = 9.14851, step = 10000
# 100W step, Loss for final step: 5.555
...
def attention_layer(from_tensor,
    ...
    if conv_type in ['sdconv']:
        ...
        unfold_conv_out_layer = tf.stack(
            [tf.slice(conv_out_layer, [0, i, 0], [batch_size, to_seq_length, num_attention_heads * size_per_head]) for i
             in range(conv_kernel_size)], -1)
        ...
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            with tf.variable_scope("intermediate"):
                if linear_groups == 1:
                    ...
                    intermediate_output = gdense(
                        tf.reshape(attention_output, [batch_size * seq_length, hidden_size]),
                        intermediate_size,
            ...
            with tf.variable_scope("output"):
                if linear_groups == 1:
                    ...
                    prev_output = gdense(
                        tf.reshape(intermediate_output, [batch_size * seq_length, intermediate_size]),
                        hidden_size,
                        groups=linear_groups,
                        kernel_initializer=create_initializer(initializer_range))
