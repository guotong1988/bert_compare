...
# global_step/sec: 1.12585
# 100W step, Loss for final step: 5.1134157
...
class BertModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            self.embedding_output3 = tf.layers.dense(
                tf.concat([self.embedding_output2, self.embedding_output], axis=-1),
                config.hidden_size,
                activation=None,
...
def transformer_model(input_tensor,
    ...
    for layer_idx in range(num_hidden_layers):
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            layer_input2 = tf.cast(tf.expand_dims(sequence_mask, -1), tf.float32) * tf.reshape(layer_input, [batch_size, seq_length, hidden_size])
            ...
            layer_input3 = tf.layers.dense(
                tf.concat([layer_input2, tf.reshape(layer_input, [batch_size, seq_length, hidden_size])], axis=-1),
                hidden_size,
                activation=None,
                kernel_initializer=create_initializer(initializer_range))
            ...
            with tf.variable_scope("attention"):
                ...
                with tf.variable_scope("self"):
                    attention_head = attention_layer(
                        from_tensor=layer_input3,
                        to_tensor=layer_input3,
                        attention_mask=attention_mask,
