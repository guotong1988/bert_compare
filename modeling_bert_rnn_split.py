...
# global_step/sec: 2.13
# loss = 9.85176, step = 10000
...
class BertModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            self.embedding_output = tf.cast(tf.expand_dims(input_mask, -1), tf.float32) * self.embedding_output
            tmp1 = my_lstm_layer(self.embedding_output[:, 1:1 + 25], is_training=is_training, scope_name="my_rnn1")[2]
            tmp2 = my_lstm_layer(self.embedding_output[:, 1 + 25:1 + 25 + 15], is_training=is_training, scope_name="my_rnn2")[2]
            tmp3 = my_lstm_layer(self.embedding_output[:, 1 + 25 + 15:1 + 25 + 15 + 23], is_training=is_training, scope_name="my_rnn3")[2]
            self.embedding_output = tf.cast(tf.expand_dims(input_mask, -1), tf.float32) * tf.concat([
                self.embedding_output[:, 0:1], tmp1, tmp2, tmp3], axis=1)
